function [X,tf] = ForwardEuler(eval_f,x_start,eval_u,p,t_start,t_stop,timestep,visualize,freq,pVisualize,otherParams,handles)
...
%    colorbar;
    visualizeNetwork(X(:,1),pVisualize,handles);
...
for n=1:ceil((t_stop-t_start)/timestep)
   ...
   disp(t(n+1));
   u = eval_u;
   f = feval(eval_f, X(:,n), u, p, 0, otherParams); 
   X(:,n+1)= X(:,n) +  dt * f;
%    if visualize
...
%        colorbar;
        ...
        visualizeNetwork(X(:,n+1),pVisualize,handles);
        count = 1;
